...
public class OADM88 extends OADM {
    public OADM88(int slot, boolean isGS) throws IOException {
        super(slot, isGS);
        dataFile = dataDir + "OADM/OADM88data.properties";
    }

    public OADM88(int slot, boolean isGS, double startCh) throws IOException {
    ...
    public void createProtocol() throws IOException {
        ...
        for (int i = 1; i < 9; i++) {
            getCell("E" + (i + 26), sheetFrom).setCellValue(get(protocolFile, "LN1" + "CH" + i + "W"));
        }
        for (int i = 1; i < 9; i++) {
            getCell("E" + (i + 36), sheetFrom).setCellValue(get(protocolFile, "CH" + i + "LN1"));
        }
        getCell("E45", sheetFrom).setCellValue(get(protocolFile, "UP1LN1"));
        for (int i = 1; i < 9; i++) {
            getCell("E" + (i + 45), sheetFrom).setCellValue(get(protocolFile, "LN1CH" + i));
        }
        getCell("E54", sheetFrom).setCellValue(get(protocolFile, "LN1UP1"));
        getCell("B82", sheetFrom).setCellValue(format(new Date()));
        getCell("E82", sheetFrom).setCellValue(engineer);
